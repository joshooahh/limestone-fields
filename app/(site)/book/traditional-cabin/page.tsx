...
export default async function TraditionalCabinPage() {
  const raw = await client.fetch<BookingImages | null>(bookingImagesQuery).catch(() => null)
  const imageUrl = raw?.traditionalCabin?.asset
    ? urlForImage(raw.traditionalCabin).width(1200).auto('format').url()
    : null
  ...
  return (
    <>
      ...
      {/* Main content */}
      <section className="bg-limestone-cream py-16 md:py-24 border-t border-[#253136]/10">
          ...
          <div className="grid gap-12 md:gap-20 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start">
            ...
            <div className="space-y-8">
              <p className="text-[18px] text-[#253136] leading-[1.6]">
                Our original cabin layout — pared back and thoughtfully designed. Everything you
                need to settle in and slow down. A king bed sleeping alcove, private outdoor cedar
                soaking tub, lake views, and a built-in workstation for those who bring their work
                with them. Nothing superfluous.
              </p>
              ...
              <div className="pt-2 space-y-4">
                <a
                  href="https://booking.limestonefields.com/listings/489934"
                  target="_blank"
